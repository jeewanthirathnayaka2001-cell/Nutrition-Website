...
Natamycin	E235	Preservative / Antimycotic agent	1. Cheese 2. Cured meats	1. Cheese: Up to 40 mg/kg / 20 ppm / 1 mg/dm² 2. Cured meats: 20 mg/kg / 1 mg/dm²	mg/kg, ppm, mg/dm²	Codex, FAO/WHO, EU, US FDA, Sri Lanka	Bacterial Neutrality, Application Method, EU Depth Rule, ADI/Safety	2025 / 2026
...
function parseLines(raw) {
    return raw.trim().split('\\n').filter(line => line.trim() !== '').map(line => {
        ...
        return {
            name: parts[0].trim(),
            eNumber: parts[1].trim(),
            functionalClass: parts[2].trim(),
            foods: parts[3].trim(),
            mpl: (parts[4].trim() + ' ' + parts[5].trim()).trim(),
            regSource: parts[6].trim(),
            notes: parts[7].trim(),
            lastUpdated: parts[8].trim()
        };
    });
}

const allData = [
    ...parseLines(rawPreservatives),
    ...parseLines(rawColours),
    ...parseLines(rawEmulsifiers),
    ...parseLines(rawAntioxidants),
    ...parseLines(rawStabilizers)
];

const jsContent = "const dbData = " + JSON.stringify(allData, null, 4) + ";\\n";
fs.writeFileSync('c:/Users/Dulanji Rathnayaka/Desktop/Nutrition-Website/data.js', jsContent, 'utf8');
